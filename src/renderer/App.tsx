...
const App = () => {
    useEffect(() => {
        function onClick(e: MouseEvent) {
            ...
            if (el.tagName === 'A' && (el as HTMLAnchorElement).href) {
                e.preventDefault();
                API.open((el as HTMLAnchorElement).href);
            }
        }
        window.addEventListener('click', onClick);
        return () => window.removeEventListener('click', onClick);
    });
    const { state: colors, reload: reloadColors } = useAsyncState(
        API.getColors
    );
    ...
    const [tab, setTab] = useState<
        'search' | 'api-docs' | 'settings' | 'proc-list' | 'package-list'
    >('search');
            ...
            stop(cb, newName) {
                return (...args) => {
                    if (newName)
                        ...
                        $setLoaderData((data) => ({ ...data, loading: false }));
                    setTimeout(
                        () =>
                            $setLoaderData((data) => ({
                                ...data,
        ...
        function keydown(e: KeyboardEvent) {
            ...
            if (e.key === 'k' && e.ctrlKey) {
                API.getProcs().then((procs) =>
                    Object.keys(procs).map((pid) =>
                        API.killProcess(Number(pid))
                    )
                );
                return e.preventDefault();
            }
    ...
    useEffect(() => {
        const a = [
            ...
            API.addEventListener(
                'display-error',
                (ev, name: string, err: string) => error(name)(err)
            ),
            API.addEventListener('err-end', () => setErrMode(false)),
            API.addEventListener(
                'loader-update',
                (ev, props: Partial<LoaderData>) =>
    ...
    useEffect(() => {
        if (inputRef.current) inputRef.current.placeholder = props.config.searchName;
        if (inputRef.current) inputRef.current.disabled = props.config.disableSearch;
        if (footerRef.current) footerRef.current.style.display =
            props.config.disableBar || footer.length < 1 ? 'none' : 'flex';
        if (tabRef.current)
            tabRef.current.style.display = props.config.disableTabs
                ? 'none'
                : 'flex';
        if (inputRef.current) inputRef.current.style.display =
            ...
            (!uTabData.enabled || uTabData.names.length < 1)
                ? 'none'
                : 'inline-block';
        if (inputRef.current) inputRef.current.style.borderBottom =
            props.config.disableTabs &&
            (!uTabData.enabled || uTabData.names.length < 1)
        ...
        if (pxMinus === 0 && remMinus === 0) calculation = '100vh';
            ...
            calculation = `calc(100vh${
                remMinus ? ' - ' + remMinus + 'rem' : ''
            }${pxMinus ? ' - ' + pxMinus + 'px' : ''})`;
    ...
    useEffect(() => {
        if (!(hasScriptFunctionality || uLoader.open) && i > 0)
            API.hideWindow();
    ...
    return (
        <div>
            <input
                type='text'
                className='script-search tabs-ignore'
                ref={inputRef}
                autoFocus
                value={search}
